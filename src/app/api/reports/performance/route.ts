...
export async function GET(request: Request) {
  try {
    ...
    const reportType = searchParams.get('type') || 'weekly'; // weekly, monthly, custom
    ...
    // Fetch acquisitions based on report type
    let acquisitionsQuery = supabase.from('acquisitions').select('*');
    
    if (reportType === 'weekly') {
      // Get current week (simplified - assuming week 1-4 per month)
      const today = new Date();
      const weekNum = Math.ceil(today.getDate() / 7);
      acquisitionsQuery = acquisitionsQuery.eq('week', weekNum);
    } else if (reportType === 'monthly' && startDate && endDate) {
      // For monthly, we still use week numbers but filter by date range
      // This is a simplified approach
    } else if (reportType === 'custom' && startDate && endDate) {
      // Custom date range - still uses week system
    }
    ...
    const teamPerformance = teams.map(team => {
      ...
      const targetAchievement = totalTarget > 0 
        ? Math.round((totalQuantity / totalTarget) * 100) 
        : 0;
    ...
    const memberPerformance = members.map(member => {
      ...
      // Target achievement per product
    ...
    const summary = {
      ...
      avgAttendanceRate: memberPerformance.length > 0
      ...
      totalLateDays: memberPerformance.reduce((sum, m) => sum + 
        (attendances.filter(a => a.member_id === m.memberId && a.status === 'late').length), 0),
      totalLeaveDays: memberPerformance.reduce((sum, m) => sum + 
        (attendances.filter(a => a.member_id === m.memberId && a.status === 'leave').length), 0),
      totalAlphaDays: memberPerformance.reduce((sum, m) => sum + 
        (attendances.filter(a => a.member_id === m.memberId && a.status === 'alpha').length), 0),
    };
    ...
    const productBreakdown = products.map(p => {
      ...
      return {
        ...
        achievementRate: p.weekly_target > 0 ? Math.round((totalQty / p.weekly_target) * 100) : 0,
      };
    }).sort((a, b) => b.totalQuantity - a.totalQuantity);

    // Attendance correlation (performance vs attendance)
    const attendanceCorrelation = memberPerformance
